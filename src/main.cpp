...
CBigNum bnProofOfWorkLimit(~uint256(0) >> 24); // "standard" scrypt target limit for proof of work, results with 0,000244140625 proof-of-work difficulty
CBigNum bnProofOfStakeLimit(~uint256(0) >> 20);
CBigNum bnProofOfWorkLimitTestNet(~uint256(0) >> 16);

unsigned int nTargetSpacing = 1 * 60; // BitStone - 1 minute blocks
...
const string strMessageMagic = "BitStone Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
	...
	if(pindexBest->nHeight+1 >= 0 && pindexBest->nHeight+1 <= 100)
    {
        ...
        else if(pindexBest->nHeight+1 >= 101 && pindexBest->nHeight+1 <= 721)
    {
        nSubsidy = 333 * COIN;
    }
        else if(pindexBest->nHeight+1 >= 722 && pindexBest->nHeight+1 <= 1440)
    {
        nSubsidy = 133 * COIN;
    }
        else if(pindexBest->nHeight+1 >= 1441 && pindexBest->nHeight+1 <= 17280)
    {
        nSubsidy = 23 * COIN;
    }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "BitStone", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "BitStone Altcoin 2015!!";
        CTransaction txNew;
        txNew.nTime = 1435675025;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1435675025;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 3289922;
		if(fTestNet)
        {
            block.nNonce   = 0;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x3bf7477a140e6a837576ee24ea5299f3ce6997df570344b20d90480839597f29"));
        block.print();
